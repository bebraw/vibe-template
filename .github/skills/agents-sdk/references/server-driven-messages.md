...
```typescript
await this.saveMessages((existingMessages) => [...existingMessages, { role: "user", content: "Check for new notifications" }]);
```
...
```typescript
await this.persistMessages([...this.messages, { role: "assistant", content: "System note: checked at " + new Date() }]);
```
