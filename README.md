...
- Verify executable Workers capability kits in isolated temporary adopters with `npm run capabilities:verify`; use `npm run capabilities:verify:browser` for the synthetic standard-adopter browser lane.
...
The `security-audit` kit exposes a pinned Cloudflare workflow for explicit, independently verified codebase audits. It keeps focused `$security` reviews in the baseline and adds no default scan or CI step.
...
The `deployment-safety` kit replaces direct deployment with an undeployed version preview, explicit version promotion, and explicit rollback. It records Cloudflare's current preview limitations and is intentionally not presented as suitable for Workers with Durable Objects.
