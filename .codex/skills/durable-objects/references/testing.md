...
describe("DO internals", () => {
  it("can verify storage directly", async () => {
    ...
    await runInDurableObject(stub, async (instance: Counter, state) => {
      ...
      const result = state.storage.sql.exec<{ value: number }>("SELECT value FROM counters WHERE name = ?", "default").one();
...
describe("SQLite", () => {
  it("can verify SQL storage", async () => {
    ...
    await runInDurableObject(stub, async (instance, state) => {
      const rows = state.storage.sql.exec<{ name: string; value: number }>("SELECT name, value FROM counters ORDER BY name").toArray();
