...
export default {
  async scheduled(controller, env, ctx) {
    ...
    const { results } = await env.DB.prepare(`SELECT date, revenue, orders FROM daily_stats WHERE date >= ? ORDER BY date`)
...
export default {
  async scheduled(controller, env, ctx) {
    ...
    try {
      ...
      ctx.waitUntil(
        env.METRICS.put(`cron:${controller.scheduledTime}`, JSON.stringify({ ...meta, status: "success" }), { expirationTtl: 2592000 }),
      );
    } catch (error) {
