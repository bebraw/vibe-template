...
description: >-
  Persistent, high-level directives that shape agent behavior and
...
references:
  ...
  - type: paper
    ...
    publisher: Anthropic
    annotation: >-
      Seminal paper by Bai et al. (2022) defining the 'Helpful, Honest,
      Harmless' framework and the RLAIF method.
  - type: website
    ...
    annotation: >-
      Defines the model for Constitutional AI: training a harmless assistant via
      self-critique based on a constitution.
  - type: website
    ...
    annotation: >-
      Provides the taxonomy for 'Steering Constraints' (Constitution) vs 'Hard
      Constraints' (Orchestration/Hook).
---
> **ASDLC Knowledge Base** | Status: Live | [View Online](https://asdlc.io/patterns/agent-constitution)
...
In the [OODA Loop](../concepts/ooda-loop.md) (Observe-Orient-Decide-Act), the Constitution lives squarely in the **Orient** phase.
...
*   *See [Agent Personas](../practices/agent-personas.md)*
...
What must the agent *never* do? These are "Soft Gates"—instructions to avoid bad paths before hitting the hard [Context Gates](../patterns/context-gates.md).
...
**[Constitutional Review](../patterns/constitutional-review.md)** — The pattern for using a Critic agent to review code specifically against the Agent Constitution.

**[Context Gates](../patterns/context-gates.md)** — The deterministic checks that back up the probabilistic Constitution. Hard Constraints implemented via orchestration.

**[Adversarial Code Review](../patterns/adversarial-code-review.md)** — Uses persona-specific Constitutions (Builder vs Critic) to create dialectic review processes.

**[The Spec](../patterns/the-spec.md)** — Defines task-specific requirements, while the Constitution defines global behavioral guidelines.

**[AGENTS.md Specification](../practices/agents-md-spec.md)** — The practice for documenting and maintaining your Agent Constitution.

**[Workflow as Code](../practices/workflow-as-code.md)** — Implements Hard Constraints programmatically, complementing the Constitution's Steering Constraints.
